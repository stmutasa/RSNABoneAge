...
import tarfile      # makes it possible to read and write .tar files
...
def backward_pass(total_loss, global_step,lr_decay=False):
    """ This function performs our backward pass and updates our gradients
    Args:
        total_loss is the summed loss caculated above
        global_step is the number of training steps we've done to this point, useful to implement learning rate decay
    Returns:
        train_op: operation for training"""
    if lr_decay == True:                                # Use decaying learning rate if flag is true
    ...
    apply_gradient_op = opt.apply_gradients(grads,global_step)  # Returns an operation that applies the gradients
